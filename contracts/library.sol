...
contract Library {
  ...
  // Mapping of Book id to the wallet address 
  ...
  function _getBookList(bool finished) private view returns (Book[] memory) {
    ...
    uint counter = 0; // if book is of onwer then counted if read or not
  ...
  function setFinished(uint bookId, bool finished) external {
    if (bookToOwner[bookId] == msg.sender) // only mapped owner of book can change state
     {
